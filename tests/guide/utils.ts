import type { Scale, Axis } from '@/types';
...
export function renderAxis({
  ...
  axis(renderer, scale, coordinate, options as any);
  return renderer.node();
}

export function firstOf(svg: SVGAElement, className: string) {
  const [node] = svg.getElementsByClassName(className);
  return {
    toBeNull() {
      expect(node).toBeUndefined();
    },
    toEqual({ textContent, ...attributes }: any) {
      const renderedAttributes = Object.keys(attributes).reduce((obj: any, key) => {
