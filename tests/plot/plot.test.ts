...
describe('plot', () => {
  ...
  test('symmetried interval', () => {
    const chart = sp.plot({
      ...
      data: sports,
      paddingLeft: 60,
      transforms: [(data) => data.sort((a: any, b: any) => b.sold - a.sold)],
  ...
  test('stacked interval in polar', () => {
    const chart = sp.plot({
      ...
      transforms: [
        (data) => {
          const sum = data.reduce((total: number, d: any) => total + (d.sold as number), 0);
          return data.map(({ genre, sold }: any) => ({ genre, sold: sold / sum }));
        },
      ],
  ...
  test('pyramid interval with transform encoding', () => {
    const chart = sp.plot({
      ...
      encodings: {
        x: 'month',
        y: (d: any) => (d.city === 'London' ? d.rainfall : -d.rainfall),
        fill: 'city',
      },
    });
      ...
      children: [
        ...
        },
        {
          ...
          encodings: {
            ...
            text: (d: any) => (d.profit > 0 ? `+${d.profit}` : d.profit),
            y: (d: any) => (d.profit > 0 ? d.end : d.start),
            fontSize: 10,
          },
          styles: {
            dy: '-0.5em',
  ...
  test('link', () => {
    const chart = sp.plot({
      ...
      data: profit,
      paddingLeft: 80,
      transforms: [(data) => data.filter((d: any) => d.month !== 'Total')],
      scales: {
        color: { type: 'identity' },
        y: { nice: true },
      },
      encodings: {
        ...
        y: 'start',
        y1: 'end',
        stroke: (d: any) => (d.profit > 0 ? 'red' : 'green'),
      },
      styles: {
        strokeWidth: 2,
      },
    ...
    const chart = sp.plot({
      ...
      children: [
        ...
        },
        {
          ...
          data: treemap,
          encodings: {
            x: (d: { x: number; x1: number }) => (d.x + d.x1) / 2,
            y: (d: { y: number; y1: number }) => (d.y + d.y1) / 2,
            text: 'name',
          },
          styles: {
            dy: '0.5em',
  ...
  test.skip('facet', () => {
    void fetch('https://gw.alipayobjects.com/os/antvdemo/assets/data/diamond.json')
  ...
  test.skip('ribbon', () => {
    void fetch('https://gw.alipayobjects.com/os/bmw-prod/b45f3425-04bc-454a-8edd-26a0b4fc28d6.json')
      // eslint-disable-next-line @typescript-eslint/promise-function-async
      .then((response) => response.json())
      .then((data) => {
        const month = [
