...
function renderViews(views: any, width = 640, height = 480) {
  const renderer = createRenderer(width, height);
  mount(createDiv(), renderer.node());
  return views.map(([{ x, y, width, height }]: any) =>
    renderer.rect({ x, y, width, height, stroke: 'black', fill: 'none' }),
  );
}
...
describe('createViews', () => {
  ...
  test('facet container with specified x', () => {
    ...
    expect(rest).toEqual({ height: 375, width: 275, x: 45, y: 60 });
    expect(node).toEqual({});
  ...
  test('facet container with specified y', () => {
    ...
    const [, [view, [node]]] = views;
    const { transform, ...rest } = view;
    expect(rest).toEqual({ height: 125, width: 550, x: 45, y: 60 });
    expect(node).toEqual({});
    expect(transform(data)).toEqual([
      { sex: 'male', skin: 'white' },
      { sex: 'female', skin: 'white' },
  ...
  test('facet container', () => {
    ...
    const [, [view, [node]]] = views;
    const { transform, ...rest } = view;
    expect(rest).toEqual({ height: 111.66666666666667, width: 265, x: 45, y: 60 });
    expect(node).toEqual({});
    expect(transform(data)).toEqual([{ sex: 'male', skin: 'white' }]);
  });
});
