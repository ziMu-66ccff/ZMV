...
function nice(
  ...
  niceDomain: [number | string, number | string],
...
function ticks(domain: [number | string, number | string], tickCount: number | string) {
  ...
  test('createLinear(options) uses custom interpolate.', () => {
    const s = createLinear({
      ...
      interpolate: (t, start, end) => {
        const charCode = interpolateNumber(t, start.charCodeAt(), end.charCodeAt());
